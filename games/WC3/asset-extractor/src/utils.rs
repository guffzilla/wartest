...
/// Utility functions for WC3 asset extraction
...
impl ExtractionUtils {
    ...
    pub fn create_temp_dir() -> Result<PathBuf> {
        let temp_dir = std::env::temp_dir().join(format!("wc3_extract_{}", 
...
impl FileStats {
    ...
    pub fn formatted_size(&self) -> String {
        ExtractionUtils::format_file_size(self.size)
    }
...
mod tests {
    ...
    fn test_ensure_output_dir() {
        ...
        assert!(ExtractionUtils::ensure_output_dir(&output_dir).is_ok());
        assert!(output_dir.exists());
    }

    #[test]
    fn test_generate_output_filename() {
        let input_path = PathBuf::from("test.mpq");
        let output_filename = ExtractionUtils::generate_output_filename(&input_path, "extracted", "txt");
        
        assert_eq!(output_filename, PathBuf::from("test_extracted.txt"));
    }
    ...
    fn test_format_file_size() {
        ...
        assert_eq!(ExtractionUtils::format_file_size(500), "500 B");
    }
    ...
    fn test_file_stats() {
        ...
        let stats = FileStats::new(4);
        assert_eq!(stats.size, 4);
        assert!(!stats.is_empty());
        assert_eq!(stats.formatted_size(), "4 B");
    }
}
