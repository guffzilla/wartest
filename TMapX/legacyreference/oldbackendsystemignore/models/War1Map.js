 ...
 */
const War1MapSchema = new Schema({
...
War1MapSchema.index({ category: 1, displayOrder: 1 });

// Index for rating sorting
War1MapSchema.index({ averageRating: -1, ratingCount: -1 });

/**
 * Add a rating to this scenario
 * @param {Object} ratingData - Object containing userId, rating, and comment
 * @returns {Promise<War1Map>} Updated scenario
 */
War1MapSchema.methods.addRating = async function(ratingData) {
 ...
 * @returns {Promise<War1Map>} Updated scenario
 */
War1MapSchema.methods.removeRating = async function(userId) {
  // Remove the rating
  this.ratings = this.ratings.filter(r => !r.userId.equals(userId));
 ...
 */
War1MapSchema.methods.getRatingByUser = function(userId) {
  return this.ratings.find(r => r.userId.equals(userId)) || null;
};

module.exports = mongoose.model('War1Map', War1MapSchema, 'war1maps'); 