...
impl AssetUtils {
    ...
    pub fn is_supported_asset(path: &Path) -> bool {
        let supported_extensions = vec!["pcx", "wav", "bin", "dat"];
...
mod tests {
    ...
    fn test_asset_utils() {
        ...
        let test_file = temp_dir.path().join("test.pcx");
        
        File::create(&test_file).unwrap().write_all(b"test").unwrap();
        
        assert!(AssetUtils::is_supported_asset(&test_file));
        assert_eq!(AssetUtils::get_asset_type(&test_file), Some(AssetType::Image));
    }

    #[test]
    fn test_asset_type() {
        assert_eq!(AssetType::Image.to_string(), "images");
